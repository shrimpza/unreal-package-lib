...
public class PackageReader implements Closeable {
	 ...
	 *                    will be kept for reuse, rather than discarded for
	 *                    potential garbage collection after moving to another
	 *                    chunk. this increases memory overhead but improves
	 *                    read performance.
	 */
	...
	public String readString() {
		...
		if (version < 64) {
			...
			int len = version > 117 ? readIndex() : (readByte() & 0xFF);
			if (len > 0) {
		...
		try {
			Supplier<ChunkChannel> chunkLoader = () -> {
				try {
					...
					if (readInt() != PKG_SIGNATURE) throw new IllegalStateException("Chunk does not seem to be Unreal package data");
	...
	public static class ReaderStats {
		...
		public String toString() {
			return String.format(
					"ReaderStats [moveToCount=%s, moveRelativeCount=%s, ensureRemainingCount=%s, fillBufferCount=%s, chunkReadCount=%s, chunkFetchCount=%s]",
					moveToCount, moveRelativeCount, ensureRemainingCount, fillBufferCount, chunkLoadCount, chunkFetchCount);
		}
	}
}
